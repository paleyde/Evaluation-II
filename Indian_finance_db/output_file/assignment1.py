...
col5=company1['comp_name'].unique()
l3=len(col5)
s3=[]
stock=company1.groupby('comp_name').agg({'NSE Symbol': "first"})
for i in range(0,l3):
    s3.append(str('STK')+str(i))
stock['stock_code']=s3    
stck=stock.set_index('stock_code').rename(columns={'NSE Symbol':'stock_name'})
